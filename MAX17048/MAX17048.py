...
class DFRobot_MAX17043():
  ...
  def read_voltage(self):
    ...
    '''
    return (1.50 * (self._read16(MAX17043_VCELL) >> 4))
...
#    bus.write_i2c_block_data(MAX17043_ADDR, reg, buf)
  ...
  def _write16(self,address,buf):
    """
    write buf to the register address
    """
    self.i2c.writeto_mem(self.MAX17043_ADDR,address,buf)

  def _read16(self,address):
    """
    reads the register at address, always returns bytearray of 2 char
    """
    return self.i2c.readfrom_mem(self.MAX17043_ADDR,address,2)
...
if __name__ == "__main__":
    time.sleep(1)
    bat = DFRobot_MAX17043()
    time.sleep(1)

    bat.begin()
    while(1):
        print(str(bat.read_voltage()))
